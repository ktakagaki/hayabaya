...
class RunConfig(runArgs: Array[String]) {
  ...
  def validatArraysFactorTwo(anArray: Array[Int]): Boolean = {
    assert(anArray.isInstanceOf[Array[Int]]) anArray.forall(_ % 2 == 0)
  }

  override def toString: String = s"arraySizes: $arraySizes, repetitions: $repetitions" + s", CPUName: $CPUName"
